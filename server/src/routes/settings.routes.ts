...
/**
 * Admin Endpoint: Get full settings (requires Admin JWT)
 */
settingsRouter.get('/', requireAdmin, async (req: Request, res: Response) => {
  try {
    const { data, error } = await supabase
      .from('settings')
      .select('value')
...
/**
 * Admin Endpoint: Save / update full settings (requires Admin JWT)
 */
settingsRouter.put('/', requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    return res.json({
      success: true,
      message: 'Pengaturan berhasil disimpan dan disinkronkan ke seluruh sistem.',
      data: merged
    });
  } catch (err: any) {
    return res.status(500).json({
